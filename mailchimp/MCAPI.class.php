...
class MCAPI {
     ...
     * @param array $options with 2 keys:  
            ...
            Field = "<strong>last_changed</strong>" : Select based on Inscrever-ser record last changed date
                Valid Op(eration): <strong>eq</strong> (is) / <strong>gt</strong> (after) / <strong>lt</strong> (before)
                Valid Values: 
                string last_campaign_sent  uses the date of the last campaign sent
                string campaign_id   uses the send date of the campaign that carriers the Id submitted - see campaigns()
            ...
            Field = "<strong>interests-X</strong>": where X is the Grouping Id from listInterestGroupings()
                ...
                Valid Values: a comma delimited string of interest groups for the list, just like you'd use in listInscrever-se() - see listInterestGroupings()
            
            Field = "<strong>mc_language</strong>": Select Inscrever-sers based on their set/auto-detected language
                Valid Op(eration): <strong>eq</strong> (=) / <strong>ne</strong> (!=) 
                Valid Values: a case sensitive language code from <a href="http://kb.mailchimp.com/article/can-i-see-what-languages-my-Inscrever-sers-use#code" target="_new">here</a>.
            ...
            Field = "<strong>default_location</strong>" : the location we automatically assign to a Inscrever-ser based on where we've seen their activity originate
            ...
            Default Field = A Merge Var. Use <strong>Merge0-Merge30</strong> or the <strong>Custom Tag</strong> you've setup for your merge field - see listMergeVars()
                Valid Op(erations): 
                 <strong>eq</strong> (=) / <strong>ne</strong> (!=) / <strong>gt</strong> (&gt;) / <strong>lt</strong> (&lt;) / <strong>like</strong> (like '%blah%') / <strong>nlike</strong> (not like '%blah%') / <strong>starts</strong> (like 'blah%') / <strong>ends</strong> (like '%blah')
                Valid Values: any string
     * @return int total The total number of Inscrever-sers matching your segmentation options
     */
    ...
    * @param array $type_opts optional - 
            ...
            For AutoResponder campaigns, this array should contain:
                ...
                boolean use_import_time whether or not imported Inscrever-sers (ie, <em>any</em> non-double optin Inscrever-sers) will receive
                array days optional used for "daily" schedules only, an array of the <a href="http://en.wikipedia.org/wiki/ISO-8601#Week_dates" target="_blank">ISO-8601 weekday numbers</a> to send on<div style="padding-left:15px"><table>
                    bool 1 optional Monday, defaults to true
                    bool 2 optional Tuesday, defaults to true
                    bool 3 optional Wednesday, defaults to true
    ...
    */
     ...
     * @return array struct of the statistics for this campaign
                ...
                int unInscrever-ses Number of email addresses in campaign that unInscrever-sed.
    ...
    /**
     * Get all unInscrever-sed email addresses for a given campaign
     *
     * @section Campaign  Stats
     *
     * @param string $cid the campaign id to pull bounces for (can be gathered using campaigns())
     * @param int    $start optional for large data sets, the page number to start at - defaults to 1st page of data  (page 0)
     * @param int    $limit optional for large data sets, the number of results to return - defaults to 1000, upper limit set at 15000
     * @return array a total of all unInscrever-sed emails and the specific emails for this page
                int total   the total number of unInscrever-ses for the campaign
                array data  the full email addresses that unInscrever-sed
                    string email the email address that unInscrever-sed
                    string reason For unInscrever-ses only - the reason collected for the unInscrever-se. If populated, one of 'NORMAL','NOSIGNUP','INAPPROPRIATE','SPAM','OTHER'
                    string reason_text For unInscrever-ses only - if the reason is OTHER, the text entered.
     */
    function campaignUnInscrever-ses($cid, $start=0, $limit=1000) {
        $params = array();
        $params["cid"] = $cid;
        $params["start"] = $start;
        $params["limit"] = $limit;
        return $this->callServer("campaignUnInscrever-ses", $params);
    }
     ...
     * @param array $order an array of information pertaining to the order that has completed. Use the following keys:
                ...
                string email_id the Email Id of the Inscrever-ser we should attach this order to (see the "mc_eid" query string variable a campaign passes)
     ...
     * @return array an array with keys listed in Returned Fields below
                    ...
                    array data the lists which matched the provided filters, including the following for 
                         ...
                         string Inscrever-se_url_short Our eepurl shortened version of this list's Inscrever-se form (will not change)
                         string Inscrever-se_url_long The full version of this list's Inscrever-se form (host will vary)
                         ...
                         array stats various stats and counts for the list - many of these are cached for at least 5 minutes
                             ...
                             double unInscrever-se_count The number of members who have unInscrever-sed from the given list.
                             ...
                             double unInscrever-se_count_since_send The number of members who have unInscrever-sed from the given list since the last campaign was sent
                             ...
                             double avg_sub_rate the average number of Inscrever-se per month for the list (empty value if we haven't calculated this yet)
                             double avg_unsub_rate the average number of unInscrever-se per month for the list (empty value if we haven't calculated this yet)
     ...
     * @return array list of merge tags for the list
                ...
                bool public Whether or not this field is visible to list Inscrever-sers
                ...
                string tag The merge tag that's used for forms and listInscrever-se() and listUpdateMember()
     ...
     * @param array $options optional Various options for this merge var. <em>note:</em> for historical purposes this can also take a "boolean"
                    ...
                    string default_value optional the default value for the field. See listInscrever-se() for formatting info. Defaults to blank
     ...
     * @return struct list of interest groups for the list
                ...
                array groups Array of the grouping options including:
                    ...
                    int Inscrever-sers total number of Inscrever-sers who have this group
     */
     ...
     * @return array list of webhooks
                ...
                array actions the possible actions and whether they are enabled
                    bool Inscrever-se triggered when Inscrever-ses happen
                    bool unInscrever-se triggered when unInscrever-ses happen
                    bool profile triggered when profile updates happen
                    bool cleaned triggered when a Inscrever-ser is cleaned (bounced) from a list
                    bool upemail triggered when a Inscrever-ser's email address is changed
                ...
                array sources the possible sources and whether they are enabled
                    bool user whether user/Inscrever-ser triggered actions are returned
     ...
     * @param array $actions optional a hash of actions to fire this Webhook for
            bool Inscrever-se optional as Inscrever-ses occur, defaults to true
            bool unInscrever-se optional as Inscrever-ses occur, defaults to true
            ...
            bool upemail optional when  Inscrever-sers change their email address, defaults to true
     ...
     * @param array $sources optional a hash of sources to fire this Webhook for
            bool user optional user/Inscrever-ser initiated actions, defaults to true
     ...
     * @return array an array of parameters for each static segment
                ...
                int member_count the total number of Inscrever-sed members currently in a segment
     ...
     *  in order to be included - this <strong>will not</strong> Inscrever-se them to the list!
     *
     * @section List Related
     * 
     * @param string $id the list id to connect to. Get by calling lists()
     ...
     *  in order to be removed - this <strong>will not</strong> unInscrever-se them from the list!
     *
     * @section List Related
     * 
     * @param string $id the list id to connect to. Get by calling lists()
    ...
    /**
     * Inscrever-se the provided email to a list. By default this sends a confirmation email - you will not see new members until the link contained in it is clicked!
     *
     * @section List Related
     *
     * @example mcapi_listInscrever-se.php
     * @example json_listInscrever-se.php        
     * @example xml-rpc_listInscrever-se.php
     *
     * @param string $id the list id to connect to. Get by calling lists()
     * @param string $email_address the email address to Inscrever-se
     * @param array $merge_vars optional merges for the email (FNAME, LNAME, etc.) (see examples below for handling "blank" arrays). Note that a merge field can only hold up to 255 bytes. Also, there are a few "special" keys:
                        ...
                        string NEW-EMAIL set this to change the email address. This is only respected on calls using update_existing or when passed to listUpdateMember(). Required to change via listBatchInscrever-se() - EMAIL takes precedence on other calls, though either will work.
     ...
     * @param bool $update_existing optional flag to control whether existing Inscrever-sers should be updated instead of throwing an error, defaults to false
     ...
     * @param bool $send_welcome optional if your double_optin is false and this is true, we will send your lists Welcome Email if this Inscrever-se succeeds - this will *not* fire if we end up updating an existing Inscrever-ser. If double_optin is true, this has no effect. defaults to false.
     ...
     */
    function listInscrever-se($id, $email_address, $merge_vars=NULL, $email_type='html', $double_optin=true, $update_existing=false, $replace_interests=true, $send_welcome=false) {
        $params = array();
        $params["id"] = $id;
        $params["email_address"] = $email_address;
        ...
        return $this->callServer("listInscrever-se", $params);
    }

    /**
     * UnInscrever-se the given email address from the list
     *
     * @section List Related
     * @example mcapi_listUnInscrever-se.php
     * @example xml-rpc_listUnInscrever-se.php
     *
     * @param string $id the list id to connect to. Get by calling lists()
     * @param string $email_address the email address to unInscrever-se  OR the email "id" returned from listMemberInfo, Webhooks, and Campaigns
     ...
     * @param boolean $send_notify flag to send the unInscrever-se notification email to the address defined in the list email notification settings, defaults to true
     * @return boolean true on success, false on failure. When using MCAPI.class.php, the value can be tested and error messages pulled from the MCAPI object (see below)
     */
    function listUnInscrever-se($id, $email_address, $delete_member=false, $send_goodbye=true, $send_notify=true) {
        $params = array();
        $params["id"] = $id;
        $params["email_address"] = $email_address;
        ...
        return $this->callServer("listUnInscrever-se", $params);
    }
     ...
     * consider using listBatchInscrever-se() with the update_existing and possible replace_interests parameter.
     *
     * @section List Related
     ...
     * @param array $merge_vars array of new field values to update the member with.  See merge_vars in listInscrever-se() for details.
    ...
    /**
     * Inscrever-se a batch of email addresses to a list at once. If you are using a serialized version of the API, we strongly suggest that you
     ...
     * @section List Related
     *
     * @example mcapi_listBatchInscrever-se.php
     * @example xml-rpc_listBatchInscrever-se.php
     *
     * @param string $id the list id to connect to. Get by calling lists()
     * @param array $batch an array of structs for each address to import with two special keys: "EMAIL" for the email address, and "EMAIL_TYPE" for the email type option (html or text). Aside from those, see listInscrever-se() for other merge var options
     * @param boolean $double_optin flag to control whether to send an opt-in confirmation email - defaults to true
     * @param boolean $update_existing flag to control whether to update members that are already Inscrever-sed to the list or to return an error, defaults to false (return error)
     * @param boolean $replace_interests flag to determine whether we replace the interest groups with the updated groups provided, or we add the provided groups to the member's interest groups (optional, defaults to true)
     ...
     */
    function listBatchInscrever-se($id, $batch, $double_optin=true, $update_existing=false, $replace_interests=true) {
        $params = array();
        $params["id"] = $id;
        $params["batch"] = $batch;
        $params["double_optin"] = $double_optin;
        $params["update_existing"] = $update_existing;
        $params["replace_interests"] = $replace_interests;
        return $this->callServer("listBatchInscrever-se", $params);
    }

    /**
     * UnInscrever-se a batch of email addresses to a list
     *
     * @section List Related
     * @example mcapi_listBatchUnInscrever-se.php
     *
     * @param string $id the list id to connect to. Get by calling lists()
     * @param array $emails array of email addresses to unInscrever-se
     * @param boolean $delete_member flag to completely delete the member from your list instead of just unsubscribing, default to false
     * @param boolean $send_goodbye flag to send the goodbye email to the email addresses, defaults to true
     * @param boolean $send_notify flag to send the unInscrever-se notification email to the address defined in the list email notification settings, defaults to false
     * @return array Array of result counts and any errors that occurred
     ...
     */
    function listBatchUnInscrever-se($id, $emails, $delete_member=false, $send_goodbye=true, $send_notify=false) {
        $params = array();
        $params["id"] = $id;
        ...
        $params["send_goodbye"] = $send_goodbye;
        $params["send_notify"] = $send_notify;
        return $this->callServer("listBatchUnInscrever-se", $params);
    }
     ...
     *
     * @param string $id the list id to connect to. Get by calling lists()
     * @param string $status the status to get members for - one of(Inscrever-sed, unInscrever-sed, <a target="_blank" href="http://eepurl.com/gWOO">cleaned</a>, updated), defaults to Inscrever-sed
     * @param string $since optional pull all members whose status (Inscrever-sed/unInscrever-sed/cleaned) has changed or whose profile (updated) has changed since this date/time - 24 hour format in <strong>GMT</strong>, eg "2013-12-30 20:30:00"
     * @param int $start optional for large data sets, the page number to start at - defaults to 1st page of data (page 0)
     ...
     * @return array Array of a the total records match and matching list member data for this page (see Returned Fields for details)
                ...
                array data the data for each member, including:
                    ...
                    date timestamp timestamp of their associated status date (Inscrever-sed, unInscrever-sed, cleaned, or updated) in GMT
                    string reason For unInscrever-ses only - the reason collected for the unInscrever-se. If populated, one of 'NORMAL','NOSIGNUP','INAPPROPRIATE','SPAM','OTHER'
                    string reason_text For unInscrever-ses only - if the reason is OTHER, the text entered.
     */
    function listMembers($id, $status='Inscrever-sed', $since=NULL, $start=0, $limit=100, $sort_dir='ASC') {
        $params = array();
        $params["id"] = $id;
        $params["status"] = $status;
        $params["since"] = $since;
     ...
     * @return array array of list members with their info in an array (see Returned Fields for details)
                int success the number of Inscrever-sers successfully found on the list
                int errors the number of Inscrever-sers who were not found on the list
                array data an array of arrays where each one has member info:
                    ...
                    string status The subscription status for this email address, either pending, Inscrever-sed, unInscrever-sed, or cleaned
                    ...
                    int member_rating the rating of the Inscrever-ser. This will be 1 - 5 as described <a href="http://eepurl.com/f-2P" target="_blank">here</a>
                    string campaign_id If the user is unInscrever-sed and they unInscrever-sed from a specific campaign, that campaign_id will be listed, otherwise this is not returned.
                    ...
                    string language if set/detected, a language code from <a href="http://kb.mailchimp.com/article/can-i-see-what-languages-my-Inscrever-sers-use#code" target="_blank">here</a>
     ...
     * @return array array of months and growth 
                ...
                int existing number of existing Inscrever-sers to start the month
                int imports number of Inscrever-sers imported during the month
                int optins number of Inscrever-sers who opted-in during the month
     */
     ...
     * @return array array of array of daily values, each containing:
                ...
                int subs number of double optin Inscrever-ses for the list
                int unsubs number of manual unInscrever-ses for the list
                int other_adds number of non-double optin Inscrever-ses for the list (manual, API, or import)
                int other_removes number of non-manual unInscrever-ses for the list (deletions, empties, soft-bounce removals)
     */
    ...
    /**
     * Retrieve the locations (countries) that the list's Inscrever-sers have been tagged to based on geocoding their IP address
     *
     * @section List Related
     *
     * @param string $id the list id to connect to. Get by calling lists()
     * @return array array of locations
                ...
                double percent the percent of Inscrever-sers in the country
                double total the total number of Inscrever-sers in the country
     */
    ...
    /**
     * Retrieve the clients that the list's Inscrever-sers have been tagged as being used based on user agents seen. Made possible by <a href="http://user-agent-string.info" target="_blank">user-agent-string.info</a>
     *
     * @section List Related
     *
     * @param string $id the list id to connect to. Get by calling lists()
     ...
     * @param array $order an array of information pertaining to the order that has completed. Use the following keys:
                string id the Order Id
                string email_id optional (kind of) the Email Id of the Inscrever-ser we should attach this order to (see the "mc_eid" query string variable a campaign passes) - either this or <strong>email</strong> is required. If both are provided, email_id takes precedence
    ...
    /**
     * Retrieve all List Ids a member is Inscrever-sed to.
     *
     * @section Helper
     * 
     ...
     * @return array An array of list_ids the member is Inscrever-sed to.
     */
     ...
     * @return array An array of chatter messages and properties
                ...
                string type The type of the message - one of lists:new-Inscrever-ser, lists:unInscrever-ses, lists:profile-updates, campaigns:facebook-likes, campaigns:facebook-comments, campaigns:forward-to-friend, lists:imports, or campaigns:inbox-inspections
